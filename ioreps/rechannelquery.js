...
async function getREChannelData(selectedDate) {
  const {
    ACT_START,
    ACT_END,
    LM_START,
    ...
    return `
      ...
      FROM (
        SELECT 
          ...
          SUM(CASE WHEN re_date BETWEEN '${ACT_START}' AND '${ACT_END}' THEN jml_re ELSE 0 END) AS ACT,
          SUM(CASE WHEN re_date BETWEEN '${LM_START}'  AND '${LM_END}'  THEN jml_re ELSE 0 END) AS LM,

          SUM(CASE WHEN re_date BETWEEN '${YTD_START}' AND '${ACT_END}' THEN jml_re ELSE 0 END) AS THIS_YTD,
          SUM(CASE WHEN re_date BETWEEN '${LAST_YTD_START}' AND '${LAST_YTD_END}' THEN jml_re ELSE 0 END) AS LAST_YTD

        FROM fmc_mirror.ih_re_summary
        WHERE region = 'EASTERN JABOTABEK'
          AND product NOT IN ('HWA')
        GROUP BY ${levelField}, \`new_channel\`, re_date
      ) base
      GROUP BY ${levelField}
    `;
  }
...
module.exports = { getREChannelData };